...
const Home = ({ darkMode, toggleMode }) => {
  ...
  const categorizeFolders = (folders) => {
    ...
    return folders.map(folder => {
      ...
      if (creationDate >= today) return { ...folder, category: 'today' };
      if (creationDate >= yesterday) return { ...folder, category: 'yesterday' };
      if (creationDate >= startOfWeek) return { ...folder, category: 'earlier this week' };
      if (creationDate >= startOfMonth) return { ...folder, category: 'earlier this month' };
      return { ...folder, category: 'never' };
    });
  };
      ...
      {selectedFolders ? gridView ?
        <div className='grid ml-9 mr-9 gap-x-5 grid-cols-3 w-[940px] gap-y-8 mt-5'>
          {categorizedFolders.map(folder => (
            <div key={folder.id} className={`flex w-full h-[50px] rounded-xl items-center ${darkMode ? 'dark-mode3' : 'light-mode3'} cursor-pointer`}>
              <TiFolderOpen size={20} className='mr-5 ml-5' /><h1>{folder.folder_name} - {folder.category}</h1>
              <SlOptionsVertical onClick={() => toggleOptions(folder.id)} size={15} className='ml-[130px] cursor-pointer' />
              {options && selectedFolderId === folder.id && (
                <div className={`w-[230px] flex flex-col gap-7 ${darkMode ? 'dark-mode3' : 'light-mode3'} ml-[70px] shadow-md mt-[260px] p-4 absolute rounded-xl ${darkMode ? 'dark-mode3' : 'light-mode2'} h-[200px] flex justify-center`}>
                  <div className='flex w-full'><h1 className='flex justify-between w-full'>Edit</h1> <CiEdit size={15} /></div>
        ...
        <div className='flex flex-col w-full h-full mt-5'>
          {categorizedFolders.map(folder => (
            <div key={folder.id} className='flex flex-col w-[940px] mb-5 justify-between ml-9 mr-9 items-center'>
              <div className='flex w-[940px] mb-5 justify-between ml-9 mr-9 items-center'>
                <TiFolderOpen size={20} className='mr-5' /><h1 className='flex justify-start'>{folder.folder_name} - {folder.category}</h1>
                <SlOptionsVertical onClick={() => toggleOptions(folder.id)} size={15} className='ml-[130px] cursor-pointer' />
              </div>
              <div className='w-full flex'><hr className='w-full'/></div>
              {options && selectedFolderId === folder.id && (
